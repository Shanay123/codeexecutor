-- Clean Schema without RLS recursion issues
...
-- RLS Policies for problems (users can only access their own)
...
-- RLS Policies for user_roles (NO RECURSION!)
-- Users can only view their own role
CREATE POLICY "Users can view their own role"
    ON user_roles FOR SELECT
    USING (auth.uid() = user_id);

-- Service role can manage all roles (used by backend)
